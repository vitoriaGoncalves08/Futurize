...
export default function Card({ index, listIndex, data }) {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://deployfuturize-production.up.railway.app/Atividade/${data.id}`,
        dataEditActivity
      );
  ...
  const fetchProjectMembers = async () => {
    try {
      const response = await axios.get(`https://deployfuturize-production.up.railway.app/Alocacao_projeto/${projectId}`);
      if (response.status === 200) {
